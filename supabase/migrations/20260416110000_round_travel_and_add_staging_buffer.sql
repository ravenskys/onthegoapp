...
as $$
  with service_config as (
    select
      ...
      -- Add 20 minutes for customer greeting / staging before wrench time.
      make_interval(mins => greatest(15, least(coalesce(p_service_duration_minutes, 60), 480)) + 20) as service_duration,
  ...
  estimated_slots as (
    select
      ...
      )::integer as travel_time_minutes,
